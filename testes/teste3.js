...
// Função para limpar todos os valores de um objeto ou array
const cleanObjectValues = (obj) => {
	if (Array.isArray(obj)) {
		return obj.map(cleanObjectValues); // Aplica recursivamente em arrays
	} else if (typeof obj === "object" && obj !== null) {
		return Object.fromEntries(
			Object.entries(obj).map(([key, value]) => [key, cleanObjectValues(value)]) // Aplica recursivamente em objetos
		);
	}
	return cleanText(obj); // Limpa texto simples
};
...
const formatData = (products) => {
	return products.map((product) => ({
		...
		details: cleanObjectValues(product.details), // Limpa todos os valores em details
	}));
};
